...
def print_table(table_scores, table_F_score, subFolder):
    total_TP = 0
    total_TP_FP = 0
    total_TP_FN = 0
    ...
    # print("%-20s%-10d%-10d%-10d%-10.4f%-10.4f%-10.4f" % (subFolder, total_TP, total_TP_FP, total_TP_FN, P, R, F), file=log)
    return [subFolder, total_TP, total_TP_FP, total_TP_FN]
    pass
    ...
    for i in range(len(table_scores)):
        ...
        if table_F_score[i][0] != 0 and table_F_score[i][1] != 0:
            table_F_score[i][2] = 2 * table_F_score[i][0] * table_F_score[i][1] / (
                    table_F_score[i][0] + table_F_score[i][1])*100
...
def evaluate(path_test, path_anno, toplevel):
    ...
    for subFolder in subFolders:
        ...
        score_by_domain = print_table(table_scores, table_F_score, subFolder)
        # print("%-20s%-10d%-10d%-10d%-10.4f%-10.4f%-10.4f" % (score_by_domain[0], score_by_domain[1], score_by_domain[2], score_by_domain[3], 0, 0, 0), file=log)
        total_scores.append(score_by_domain)
    ...
    # print_table(total_scores, table_F_score, "All")
...
if __name__ == '__main__':
    ...
    path_out = "E:\VLSP\Evaluate1\Results"
    teams = os.listdir(path)
    for team in teams:
        print(team)
        path_test = os.path.join(path, team)
        ...
        log = io.StringIO()
        titles = list_tests
        print("Domain\t" + "\t".join(titles), file=log)
        list_domain = ["batdongsan", "congnghe", "doisong", "dulich", "giaitri", "giaoduc", "gocnhin", "khoahoc",
                       "kinhdoanh", "kinhte", "nhanai", "phapluat", "suckhoe", "sukien", "thegioi", "thethao", "thoisu",
                       "truyenhinh", "vanhoa", "vieclam", "xahoi"]

        overall = np.array(list_domain)
        for i in range(len(list_tests)):
            ...
            top = np.array(total_scores_top)[:,1:].astype(np.int)
            nested = np.array(total_scores_nested)[:,1:].astype(np.int)

            # print(list_domain, top+nested)
            table_scores = list(top+nested)

            F_table_scores = np.array(get_scores(table_scores))[:,2]
            overall = np.c_[overall, F_table_scores]
        ...
        f_out = open(path_out+"/"+team+"_"+"result.txt", "w", encoding="utf-8")
