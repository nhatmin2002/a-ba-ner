...
if __name__ == '__main__':
    path_anno = "E:\VLSP\Evaluate0\Annot"
    path_out = "E:\VLSP\Evaluate0\Results"
    path = "E:\VLSP\Evaluate0\CacDoiSubmit"
    teams = os.listdir(path)
    for team in teams:
        print(team)
        path_team = os.path.join(path, team)
        tests = os.listdir(path_team)
        for test in tests:
            path_test = os.path.join(path_team, test)
            if os.path.isdir(path_test):
                ...
                print("Evaluate completed!", file=log)
                f_out = open(path_out+"/"+ team +"/"+test+".txt", "w", encoding="utf-8")
                text = log.getvalue()
                print(text)
                f_out.write(text)